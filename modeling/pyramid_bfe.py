...
# ft_net_50_1
class PyramidBFE(nn.Module):

    def __init__(self, class_num):
        super(PyramidBFE, self).__init__()
        ...
        self.classifier_local = nn.Linear(self.reduct_dim, class_num, bias=False)
    ...
    def forward(self, x):
        ...
        local_featuremap = self.bottleneck(x)  #(b,2048,h,w)
        ...
        l_feat = local_feat.view(local_feat.shape[0], -1)
        ...
        if self.training:
            return [x16, x18, x22, cls_score_local], [x_0, x_1, x3, x_3, x_10, x_11]
        else:
            #return torch.cat((x15,x17,x21),1)
            return torch.cat((x14,x19,x23,x_0,x_1,local_feat),1)
