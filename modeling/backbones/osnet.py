    ...
    def load_param(self, model_path):
        ...
        if len(matched_layers) == 0:
            warnings.warn(
                ...
                '(** ignored and continue **)'.format(model_path))
        else:
            print('Successfully loaded pretrained weights from "{}"'.format(model_path))
...
def osnet_x1_0(**kwargs):
    ...
    return OSNet( blocks=[OSBlock, OSBlock, OSBlock], layers=[2, 2, 2],
                 channels=[64, 256, 384, 512], **kwargs)



def osnet_ibn_x1_0( **kwargs):
